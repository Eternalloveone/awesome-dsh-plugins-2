...
Usage:
  ...
  python3 scripts/aggregate.py --render-only  # rebuild CATALOG.md from current CSV
"""
...
VERIFIED_CAT_MAP = {
    ...
    "game": "fun",
    # Categories emitted by the bounded review agents.
    "ui": "web_ui", "other": "uncategorized", "memory": "memory_persona",
    "mcp": "developer_tools", "knowledge": "data_tools", "analysis": "data_tools",
    "agent_os": "automation", "agent-os": "automation", "security": "diagnostics",
    "legal": "ecosystem", "desktop": "remote_device", "prompt": "developer_tools",
    "launcher": "remote_device", "list": "ecosystem",
...
def normalize_catalog_category(record):
    category = (record.get("category") or "").strip()
    if category in CATS:
        return category
    mapped = VERIFIED_CAT_MAP.get(category)
    return mapped if mapped in CATS else keyword_category(record)


def load_catalog_records(path):
    """Load the existing aggregate table without making network requests."""
    records = []
    seen = set()
    for row in read_csv(path):
        name = (row.get("full_name") or "").strip()
        if not name:
            raise ValueError(f"{path}: empty full_name")
        key = name.lower()
        if key in seen:
            raise ValueError(f"{path}: duplicate full_name {name!r}")
        seen.add(key)
        raw_stars = (row.get("stars") or "").strip()
        try:
            stars = 0 if raw_stars.upper() == "N/A" else int(raw_stars or 0)
        except ValueError as exc:
            raise ValueError(f"{path}: invalid stars for {name!r}") from exc
        record = {
            **row,
            "full_name": name,
            "html_url": row.get("html_url") or f"https://github.com/{name}",
            "stars": stars,
            "topics": [item for item in (row.get("topics") or "").split("|") if item],
            "verified": (row.get("verified") or "").strip().lower() == "true",
        }
        record["category"] = normalize_catalog_category(record)
...
def render_catalog(records):
    ...
    by_cat = OrderedDict((category, []) for category in CATS)
    for source in records:
        record = dict(source)
        record["category"] = normalize_catalog_category(record)
        record["verified"] = (
            record.get("verified") if isinstance(record.get("verified"), bool)
            else (record.get("verified") or "").strip().lower() == "true"
        )
        record["stars"] = (
            record.get("stars") if isinstance(record.get("stars"), int)
            else int(record.get("stars") or 0)
        )
    ...
    lines = [
        ...
        "",
        (
            "> This page aggregates **every** repository found under the GitHub `dsh-plugin` / "
        ...
        "",
        f"**合计 / Total: {len(records)}**（已核验 / Verified: "
        f"{sum(bool(record.get('verified')) for record in records)}）",
        "",
    ]
    for category, items in by_cat.items():
        zh, en = CATS[category]
        lines.extend([f"## {zh} / {en} — {len(items)}", ""])
        for record in sorted(
                items,
                key=lambda item: (
                    not item["verified"], -item.get("stars", 0),
            ...
            metadata = []
            if record.get("stars"):
                metadata.append(f"⭐{record['stars']}")
            if record.get("license"):
                metadata.append(record["license"])
            ...
            lines.append(
                ...
                f"{description}{metadata_text}{flag}"
            )
        lines.append("")
    return "\n".join(lines), len(records), sum(
        bool(record.get("verified")) for record in records
    ), by_cat


def write_catalog(records, path):
    content, total, verified_n, by_cat = render_catalog(records)
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(content)
    return total, verified_n, by_cat, len(content.splitlines())
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="render CATALOG.md from data/repositories.csv without network access",
    )
...
def main(argv=None):
    ...
    if args.render_only:
        ...
        records = load_catalog_records(csv_path)
        total, verified_n, _, line_count = write_catalog(records, catalog_path)
        print(f"wrote: CATALOG.md ({line_count} lines)")
    ...
    # 8) write CATALOG.md
    catalog_path = os.path.join(ROOT, "CATALOG.md")
    total, verified_n, by_cat, line_count = write_catalog(
        merged.values(), catalog_path
    )
